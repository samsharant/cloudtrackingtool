...
function CostDropdown({
  ...
  const [selectedMainOption, setselectedMainOption] = useState("cost");
  ...
  const filterNumericValue = parseFloat(filterValue).toFixed(2);
            ...
            items: [
              {
                field: "serviceCost",
                operator: "=",
                value: parseFloat(filterNumericValue),
              },
            ],
          });
          setSelectedOption(
    ...
    <div style={{ marginLeft: "20px" }}>
      <Tooltip title="Cost">
        <Button
          variant="outlined"
          sx={{
            backgroundColor: filterNo === 3 ? "#f5d6b5" : "#f5f5f5",
            color: "#000000",
            borderColor: "#eeeeee",
            fontSize: "12px",
            fontWeight: "600",
        ...
        >
          Cost <ArrowDropDownIcon fontSize="small" />
        </Button>
      </Tooltip>
      <Popover
      ...
      >
        <>
          <List>
            {!(selectedSecondOption == "cost") && (
              <ListItem button onClick={() => setSelectedSecondOption("cost")}>
                <ListItemIcon>
                  <ArrowBackIcon />
                </ListItemIcon>
